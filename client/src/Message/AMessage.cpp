/*
** EPITECH PROJECT, 2025
** dufos
** File description:
** AMessage
*/

#include "Message/AMessage.hpp"
#include "Bytes.hpp"

Dufos::Message::AMessage::Type Dufos::Message::AMessage::getType() const
{
    return _type;
}

bool Dufos::Message::AMessage::execute(Client& client, std::uint32_t id)
{
    (void) client;
    (void) id;
    return false;
}

bool Dufos::Message::AMessage::process(Network::Data::Message& message, Client& client, std::uint32_t id)
{
    (void) message;
    (void) client;
    (void) id;
    return false;
}

void Dufos::Message::AMessage::setMessageSize(std::vector<std::uint8_t>& msg, std::uint32_t size)
{
    std::vector<std::uint8_t> sizeBytes = Network::Bytes::numberToBytes(size, 4);

    msg.insert(msg.end(), sizeBytes.begin(), sizeBytes.end());
}
