...
namespace Engine::Ecs::Component
{
    ...
    Sprite::Sprite(const std::string& path, std::uint16_t frameWidth, std::uint16_t frameHeight, std::uint16_t frameCount, float frameDuration, std::uint8_t columns)
    ...
    std::uint16_t Sprite::getSpriteWidth() const
    {
        if (_animated) {
            return _frameWidth;
        }
    ...
    std::uint16_t Sprite::getSpriteHeight() const
    {
        if (_animated) {
            return _frameHeight;
        }
    ...
    void Sprite::setFrameSize(std::uint16_t width, std::uint16_t height, std::uint16_t count, float duration, std::uint8_t cols)
    {
        _animated = true;
        _frameWidth = width;
        _frameHeight = height;
