...
public:
   ...
   */
  enum class Type { ON_CONNECTION, ON_DISCONNECTION, ON_MESSAGE_RECEPTION };
   ...
   */
  void onMessageReception(Enum::Connection::Type type, std::uint32_t clientId,
   ...
   * @param func The function to set
   */
  void setOnMessageReception(std::function<void(Enum::Connection::Type,
                                                std::uint32_t, Data::Message &)>
                                 func);

private:
  std::function<void(std::uint32_t)>
      _onConnection; /*!> The function to call when a clients connect (to be
                        defined by the user) */
  std::function<void(std::uint32_t)>
      _onDisconnection; /*!> The function to call when a clients disconnect (to
                           be defined by the user) */
  std::function<void(Enum::Connection::Type, std::uint32_t, Data::Message &)>
      _onMessageReception; /*!> The function to call when a message is received
